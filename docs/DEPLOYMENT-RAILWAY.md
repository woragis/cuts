...
├── service: worker-ffmpeg       (réplicas 4–16 manual)
...
Funcionalmente, **3 réplicas de `worker-ffmpeg` no Railway ≈ 3 pods** consumindo a mesma fila Redis — padrão correto para nosso worker.
...
**Recomendação fase 1:** monitorar lag da fila `cuts:jobs:ffmpeg` no Grafana/metrics; aumentar réplicas manualmente nos horários de pico (tarde/noite após lives).
...
| `worker-ffmpeg` | `backend/Dockerfile.worker` | **4–16** | `WORKER_STAGE=render` |
...
- Script simples: se `LLEN cuts:jobs:ffmpeg` > 50 → alerta Discord + aumentar réplicas via API
- S3 para storage
...
| worker-ffmpeg | **AWS Batch / ECS / K8s + KEDA** |
...
```text
1. LLEN cuts:jobs:ffmpeg (Redis)
2. Se > threshold_high → Railway API: worker-ffmpeg replicas = N+1
...
- [ ] Filas Redis separadas (`cuts:jobs:ffmpeg`, etc.)
